...
The \ckey{programs} table is a table of table; each of the entries (\aka
...
As well as the top-level keys, some available keys in \ckey{programs} table
...
should be applicable for other programs. The format specifiers are available
for this key.
\end{confkey}
...
that normally have \code{.aux} extensions, contains
...
are invoked only when it exists. This filename is also used for replacing value
for the format specifiers. When this key is not specified, the input filename
that is given as a command-line argument, or the filename in the value for the
\ckey{source} key is used as target.
\end{confkey}
